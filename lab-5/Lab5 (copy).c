    ...
    while(1){
    	...
    	if (strncmp(buf, "WHOIS", 5) == 0) {
    		...
    		if (isMaster){
    			...
    			char msg[MSG_SIZE];
    			sprintf(msg, "& Haoxiang:%s is Master\n", ip_addr);
    			...
    			from.sin_addr.s_addr = inet_addr("128.206.19.255");
    			if (sendto(server_fd, "& ", 2, 0, (struct sockaddr*)&from, fromlen) < 0) {
					error("sendto");
				}
    		}
			...
			if(atoi(arr[2]) > master_num) {
    	...
    	} else if (strncmp(buf, "Master Vote Result:", 19) == 0) {
    			char delim[] = ":";
				char* ptr = strtok(buf, delim);
				char* a[2];
				int i = 0;
				...
				if(strncmp(a[1], ip_addr, 14) == 0){
					isMaster = true;
					printf("You are the Master now!\n");
				}
		...
		if (remain == 0){
				...
				printf("VA: %d\n", master_num);
				char msg[MSG_SIZE];
    			sprintf(msg, "Master Vote Result:%s\n", master_ip);
    			from.sin_addr.s_addr = inet_addr("128.206.19.255");
				if (sendto(server_fd, msg, 40, 0, (struct sockaddr*)&from, fromlen) < 0) { error("sendto"); }
			}
		...
		while (strncmp(buf, "#", 1) == 0) {
    		...
    		// read next one
    		if (recvfrom(server_fd, buf, MSG_SIZE, 0, (struct sockaddr*)&from, &fromlen) < 0){ error("recvfrom"); }
    		if (strncmp(buf, "#", 1) != 0){
    			printf("IP: %s\n", master_ip);
				printf("VA: %d\n", master_num);
    		}
    	...
    	// sending
    	if (sendto(server_fd, "Got your message\n", 17, 0, (struct sockaddr*)&from, fromlen) < 0) {
    		error("sendto");
    	}
    	*/
    }
